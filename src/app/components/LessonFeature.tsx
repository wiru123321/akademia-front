...
function RenderIcon({ icon }: { icon: number }) {
  switch (icon) {
    ...
    case 6:
      return <RiNumber7 size={30} color="white" />;
    case 7:
      return <RiNumber8 size={30} color="white" />;
    case 8:
      return <RiNumber9 size={30} color="white" />;
    default:
      return null;
  }
}
...
export default function LessonFeature({ data }: LessonFeatureProps) {
  return (
    <section className="py-8 overflow-hidden opacity-0 opacity-0 intersect:animate-fade animate-duration-[1500ms] intersect-once">
      <div className="container px-4 mx-auto">
        ...
        <div className="flex flex-wrap -m-8">
          {data.item.map((item, index) => (
            <div className="w-full md:w-1/2 lg:w-1/3 p-8" key={index}>
