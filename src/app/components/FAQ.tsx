...
export default function FAQ({
  ...
  return (
    <section className="mx-auto pb-8 bg-[#CFB0AE] overflow-hidden opacity-0 intersect:animate-fade animate-duration-[1500ms] intersect-once">
              ...
              <div className="divide-y divide-slate-200">
                {lessonItems.map((faq, index) => (
                  <FAQItem
                    ...
                    title={faq.title}
                    id={`faqsl-${index}`}
                    active={activeElement === `faqsl-${index}`}
              ...
              <div className="divide-y divide-slate-200">
                {trainingItems.map((faq, index) => (
                  <FAQItem
                    ...
                    value={`faqst-${index}`}
                    id={`faqst-${index}`}
                    active={activeElement === `faqst-${index}`}
                    setActive={setActiveElement}
                  >
                    {faq.content}
                  </FAQItem>
